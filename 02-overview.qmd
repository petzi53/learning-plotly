...
If we assign variable names (e.g., `cut`, `clarity`, etc.) to visual properties (e.g., `x`, `y`, `color`, etc.) within `plotly::plot_ly()`, as done in @fig-first-try, the program tries to find a sensible geometric representation of that information for us.
...
::::{.my-r-code-container}
```{r}
#| label: fig-wrong-map-data-values
...
::::{.my-r-code-container}
```{r}
#| label: fig-correct-map-data-values
...
In addition to `plotly::layout()` for adding/modifying part(s) of the graph’s layout in @fig-using-pipe, there are also a family of `plotly:add_*()` functions (e.g., `plotly::add_histogram()`, `plotly::add_lines()`, etc) that define how to render data into geometric objects. 
...
::::{.my-r-code-container}
```{r}
#| label: fig-implicite-histogram-layer
...
::::{.my-r-code-container}
```{r}
#| label: fig-combine-multiple-layers
...
![plotly workflow from R code to the graph generated in the web browser by plotly.js](img/plotly.js-working-min.png){#fig-plotly-js 
...
::::{.my-r-code-container}
```{r}
#| label: fig-diamond-clarity-levels
...
::::{.my-r-code-container}
```{r}
#| label: fig-compare-price-per-cut-level
...
The second layer of @fig-sina-plot uses `ggplot2::stat_summary()` to overlay a 95% confidence interval estimated via a bootstrap algorithm via the {**Hmisc**} package [@Hmisc].
...
1. The ilustration with `GGally::ggcoef()` (@fig-ggally-ggcoef) does not show up in the HTML page because of memory problems.
...
@fig-ggally-ggcoef shows how injecting interactivity into this plot allows us to query exact values and zoom in on the most interesting regions. See [video](https://vimeo.com/322362701) how to do it.
...
@fig-naniar-missing-points demonstrates this by introducing fake missing values to the diamond price.
...
::::{.my-r-code-container}
```{r}
#| label: fig-naniar-missing-points
